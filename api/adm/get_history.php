<?php  
require '../config_api.php';  

if (file_exists(__DIR__ . '/../../../../../.env')) {
    require_once __DIR__ . '/../../vendor/autoload.php';
    $dotenv = Dotenv\Dotenv::createImmutable(__DIR__ . '/../../../../../');
    $dotenv->load();
}

try {     
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {         
        throw new Exception('Método não permitido');     
    }      

    $input = json_decode(file_get_contents('php://input'), true);
    $filter = isset($input['filter']) ? $input['filter'] : 'Todos';
    $limit = isset($input['limit']) ? (int)$input['limit'] : 100;
    $offset = isset($input['offset']) ? (int)$input['offset'] : 0;

    $query = "         
        SELECT              
            hc.id,             
            hc.quantidade,             
            hc.categoria as type,             
            hc.detalhes as details,             
            hc.data_adicao as date,             
            u.username as studentName,             
            u.nome_completo as fullName,             
            u.rm as studentRM,
            u.id as user_id
        FROM historico_creditos hc         
        JOIN users u ON hc.user_id = u.id
    ";      

    $params = [];
    $whereConditions = [];

    switch ($filter) {         
        case 'Doações':             
            $whereConditions[] = "hc.categoria = 'Doacao'";
            break;         
        case 'Atividades':             
            $whereConditions[] = "hc.categoria = 'Atividade'";
            break;         
        case 'Últimos 30 dias':             
            $whereConditions[] = "hc.data_adicao >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
            break;         
        case 'Todos':         
        default:             
            break;     
    }

    if (!empty($whereConditions)) {
        $query .= " WHERE " . implode(' AND ', $whereConditions);
    }

    $query .= " ORDER BY hc.data_adicao DESC LIMIT :limit OFFSET :offset";

    $stmt = $pdo->prepare($query);
    
    $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
    $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
    
    foreach ($params as $key => $value) {         
        $stmt->bindValue($key, $value);     
    }          

    $stmt->execute();     
    $history = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $countQuery = "
        SELECT COUNT(*) as total
        FROM historico_creditos hc         
        JOIN users u ON hc.user_id = u.id
    ";

    if (!empty($whereConditions)) {
        $countQuery .= " WHERE " . implode(' AND ', $whereConditions);
    }

    $countStmt = $pdo->prepare($countQuery);
    $countStmt->execute();
    $totalRecords = $countStmt->fetch(PDO::FETCH_ASSOC)['total'];

    $formattedHistory = [];     
    foreach ($history as $item) {         
        $formattedHistory[] = [             
            'id' => $item['id'],             
            'date' => $item['date'],             
            'studentName' => $item['fullName'] ?: $item['studentName'],              
            'studentRM' => $item['studentRM'],
            'user_id' => $item['user_id'],
            'credits' => (int)$item['quantidade'],             
            'type' => $item['type'] === 'Doacao' ? 'Doação' : 'Atividade',             
            'details' => $item['details'],             
            'quantity' => ''         
        ];     
    }      

    echo json_encode([         
        'success' => true,         
        'message' => 'Histórico de todos os usuários recuperado com sucesso',         
        'history' => $formattedHistory,         
        'total' => (int)$totalRecords,
        'returned' => count($formattedHistory),
        'limit' => $limit,
        'offset' => $offset
    ]);  

} catch (Exception $e) {     
    http_response_code(400);     
    echo json_encode([         
        'success' => false,         
        'message' => $e->getMessage()     
    ]); 
} catch (Error $e) {     
    http_response_code(500);     
    echo json_encode([         
        'success' => false,         
        'message' => 'Erro interno do servidor'     
    ]); 
} 
?>